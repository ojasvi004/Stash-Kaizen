...
const menuSections: { title: string; items: { icon: any; label: string; href: string; roles: string[] }[] }[] = [
  {
    ...
    items: [
      ...
      { icon: RefreshCw, label: "Smart Restock", href: "/dashboard/restock", roles: ["admin", "worker"] },
  ...
  return (
    <aside className={`sidebar ${collapsed ? "sidebar--collapsed" : ""}`}>
      ...
      >
        ...
        {!collapsed && (
          <div className="sidebar__profile-meta" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p className="sidebar__name" style={{ margin: 0 }}>{userName}</p>
              <Link 
                href="/dashboard/notifications" 
                onClick={(e) => e.stopPropagation()} 
                style={{ color: 'var(--color-brand-600)', padding: '0.25rem', borderRadius: '50%', backgroundColor: 'var(--color-brand-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                title="Notifications"
              >
                <LuBell size={16} />
              </Link>
