...
export default function WorkerDashboardView() {
  ...
  const getAiInsight = () => {
    const s = data?.stats;
    if (!s) return "Analyzing your daily tasks...";
    let insight = "";
    if (s.pending_tasks > 0) {
      insight += `You have ${s.pending_tasks} pending tasks to complete today. `;
    } else {
      insight += `You've cleared all your tasks for today! `;
    }
    if (s.low_stock_count > 0) {
      insight += `Be aware of ${s.low_stock_count} low-stock items in your bay. `;
    }
    if (s.active_deliveries > 0) {
      insight += `${s.active_deliveries} shipments need attention.`;
    }
    return insight.trim() || "You are all caught up.";
  };
        ...
        <div className="worker-dashboard__main">
          <div className="dashboard-card worker-dashboard__voice-card">
            ...
            <div className="worker-dashboard__ai-summary" style={{ marginBottom: 14, padding: "0.85rem 1rem", borderRadius: 16, border: "1px solid rgba(52,168,83,0.14)", background: "linear-gradient(135deg, rgba(52,168,83,0.08), rgba(255,255,255,0.96))" }}>
              <p style={{ margin: 0, fontSize: 12, color: "var(--color-muted)", textTransform: "uppercase", letterSpacing: 1 }}>Overall AI Insights</p>
