...
export default function AdminDashboard() {
  ...
  const getAiInsight = () => {
    if (!stats) return "Analyzing overall business performance...";
    let insight = "";
    if (stats.critical_count > 0) {
      insight += `Critical: ${stats.critical_count} items need immediate restocking. `;
    } else if (stats.low_stock_count > 0) {
      insight += `Inventory is stable, but ${stats.low_stock_count} items are running low. `;
    } else {
      insight += `All inventory levels are healthy. `;
    }
    if (stats.deliveries_in_transit > 0) {
      insight += `Operations are active with ${stats.deliveries_in_transit} deliveries in transit. `;
    }
    if (stats.active_orders > 0) {
      insight += `${stats.active_orders} orders are currently being processed.`;
    }
    return insight.trim() || "All systems operational.";
  };

  const aiSummary = getAiInsight();
    ...
    <div className="dashboard-wrapper">
      ...
      <div className="dashboard-card" style={{ marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, border: "1px solid rgba(107,66,38,0.14)", background: "linear-gradient(135deg, rgba(107,66,38,0.08), rgba(255,255,255,0.96))" }}>
        <div style={{ minWidth: 0 }}>
          <p style={{ margin: 0, fontSize: 12, color: "var(--color-muted)", textTransform: "uppercase", letterSpacing: 1 }}>Overview Summary</p>
          <p style={{ margin: "0.25rem 0 0", fontWeight: 700, color: "var(--color-brand-800)" }}>{aiSummary}</p>
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--color-brand-600)", whiteSpace: "nowrap" }}>Ask Stash AI bot</div>
      </div>
      ...
      <div className="grid-3">
        ...
        <div className="dashboard-card">
          ...
          <div style={{ height: '180px', width: '100%', minWidth: 0, minHeight: 0 }}>
