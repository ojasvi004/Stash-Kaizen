...
import { fetchOrders, RecentOrder, createOrder, fetchInventory, API_BASE } from "@/lib/api";
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      if (event.data === "orders_updated") {
        loadOrders();
      } else if (event.data === "inventory_updated") {
        fetchInventory().then((data) => setInventory(data as any[])).catch(console.error);
      }
    };
              ...
              {loading ? (
                ...
                return (
                  <tr key={order.id}>
                    ...
                    <td>
                      <p style={{ fontWeight: 500, color: 'var(--color-brand-800)' }}>{(order as any).product_name || "N/A"}</p>
                    </td>
                    <td>
                      <div>
