...
import { fetchInventory, createInventoryItem, StockItem, API_BASE } from "@/lib/api";
...
export default function InventoryPage() {
  ...
  const loadInventory = async () => {
    try {
      ...
      for (const item of data as StockItem[]) {
      ...
      const aggregatedData = Array.from(aggregatedMap.values()).map(item => {
        if (item.current_stock <= item.threshold * 0.5) item.status = "critical";
        else if (item.current_stock <= item.threshold) item.status = "low";
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${protocol}//${host}/api/ws/updates`);
