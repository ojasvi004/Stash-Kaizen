...
# Propagate webhook settings to the app process so startup registration uses the same URL.
os.environ["TELEGRAM_WEBHOOK_URL"] = webhook_url
os.environ.setdefault("ENABLE_TELEGRAM_WEBHOOK", "True")

payload = {"url": webhook_url}
if os.getenv("TELEGRAM_WEBHOOK_SECRET"):
    payload["secret_token"] = os.getenv("TELEGRAM_WEBHOOK_SECRET")

res = requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/setWebhook", data=payload)
try:
