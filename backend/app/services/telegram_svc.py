...
from app.services.gemini import classify_telegram_text_query, extract_intent_and_entities, rewrite_in_language
...
async def _build_order_analytics_reply() -> str:
    ...
    supplier_count = int(stats.get("supplier_count") or 0)
    buyer_count = int(stats.get("buyer_count") or 0)
    voice_calls_today = int(stats.get("voice_calls_today") or 0)

    lines = ["<b>Here\'s your business analytics snapshot</b>", ""]
    lines.append(
    ...
    lines.append(
        f"Inventory value is around <b>₹{inventory_value:,.2f}</b> across <b>{total_products}</b> products."
    )
    lines.append(
        f"I also see <b>{deliveries_in_transit}</b> deliveries in transit, <b>{low_stock_count}</b> low-stock items, and <b>{critical_count}</b> critical items."
    )
    lines.append(
        f"Suppliers: <b>{supplier_count}</b>, buyers: <b>{buyer_count}</b>, and <b>{voice_calls_today}</b> voice commands today."
    )
    lines.append(f"Inventory and operations are showing a live snapshot for decision-making.")
    ...
    lines.append("")
    lines.append("If you want, I can also break this down by revenue, inventory value, low-stock risk, or recent orders.")
    return "\n".join(lines)


async def _reply_in_language(text: str, language_code: str | None) -> str:
    code = (language_code or "en").strip().lower()
    if code in {"", "en", "eng", "english"}:
        return text

    try:
        return await rewrite_in_language(text, code)
    except Exception as exc:
        logger.warning("Failed to rewrite Telegram reply in %s: %s", code, exc)
        return text


def _fallback_text_intent(text: str) -> dict:
    normalized = _normalize_query_text(text)
    if _looks_like_stock_add_query(normalized):
        return {"intent": "inventory_add", "entities": {}, "reply_language": "en", "confidence": 0.5}
    if _looks_like_inventory_query(normalized):
        return {"intent": "inventory_query", "entities": {"search_term": _extract_inventory_search_term(normalized)}, "reply_language": "en", "confidence": 0.5}
    if _looks_like_order_analytics_query(normalized):
        return {"intent": "analytics_query", "entities": {}, "reply_language": "en", "confidence": 0.5}
    if _looks_like_delivery_query(normalized):
        return {"intent": "delivery_query", "entities": {}, "reply_language": "en", "confidence": 0.5}
    if _looks_like_alert_query(normalized):
        return {"intent": "alert_query", "entities": {}, "reply_language": "en", "confidence": 0.5}
    return {"intent": "unknown", "entities": {}, "reply_language": "en", "confidence": 0.0}
...
async def handle_telegram_webhook(update: dict) -> None:
    ...
    classified = None
    try:
        classified = await classify_telegram_text_query(text)
    except Exception as exc:
        logger.warning("Telegram text classification failed, using fallback heuristics: %s", exc)
        classified = _fallback_text_intent(text)

    intent = str(classified.get("intent", "unknown") or "unknown").strip().lower()
    entities = classified.get("entities", {}) or {}
    reply_language = classified.get("reply_language") or classified.get("language_detected") or "en"

    if intent in {"inventory_add", "stock_arrival"}:
        await _handle_stock_addition(int(chat_id), text)
        return

    if intent in {"analytics_query", "order_query"} or _looks_like_order_analytics_query(text):
        reply = await _build_order_analytics_reply()
        await send_message(chat_id, await _reply_in_language(reply, reply_language))
        return

    if intent == "delivery_query" or _looks_like_delivery_query(text):
        reply = await _build_deliveries_reply()
        await send_message(chat_id, await _reply_in_language(reply, reply_language))
        return

    if intent in {"inventory_query", "stock_query"} or _looks_like_inventory_query(text):
        search_term = entities.get("search_term") or _extract_inventory_search_term(text)
        items = await _fetch_inventory_items(search_term=search_term, limit=8)
        reply = _format_inventory_reply(items, search_term)
        await send_message(chat_id, await _reply_in_language(reply, reply_language))
        return

    if intent == "alert_query" or _looks_like_alert_query(text):
        alerts = await _build_prediction_alert_candidates(limit=5)
        reply = _format_prediction_alerts(alerts)
        await send_message(chat_id, await _reply_in_language(reply, reply_language))
        return

    if intent == "help":
        reply = (
            "I can check your orders, revenue, inventory, deliveries, and low-stock alerts. "
            "Try asking naturally, like 'tell me order analytics' or 'how much rice is left?'."
        )
        await send_message(chat_id, await _reply_in_language(reply, reply_language))
        return

    if intent == "greeting":
        reply = "Hello. You can ask me about inventory, analytics, deliveries, alerts, or add stock in plain language."
        await send_message(chat_id, await _reply_in_language(reply, reply_language))
        return
...
async def register_telegram_webhook() -> dict:
    ...
    configured_url = (settings.effective_backend_url or os.getenv("RENDER_EXTERNAL_URL", "")).rstrip("/")
    ...
    if not webhook_url:
        logger.warning(
            "Telegram webhook registration skipped: no public BACKEND_URL/TELEGRAM_WEBHOOK_URL/RENDER_EXTERNAL_URL configured."
        )
        return {"ok": False, "description": "Missing public webhook URL"}

    parsed_webhook = urlparse(webhook_url)
    hostname = (parsed_webhook.hostname or "").lower()
    if parsed_webhook.scheme != "https" or hostname in {"localhost", "127.0.0.1", "::1"}:
        logger.warning(
            "Telegram webhook is configured with a non-public URL (%s). Telegram requires a publicly reachable HTTPS endpoint, so incoming replies are skipped until you expose one.",
            webhook_url,
        )
        return {"ok": False, "description": "Webhook URL is not a public HTTPS endpoint"}
...
async def delete_telegram_webhook() -> dict:
    """Clear any existing Telegram webhook before switching to polling."""
    if not settings.TELEGRAM_BOT_TOKEN:
...
async def poll_telegram_updates(stop_event: asyncio.Event | None = None) -> None:
    """Fallback Telegram receiver for local development when webhooks are unavailable."""
    if not settings.TELEGRAM_BOT_TOKEN:
        logger.warning("Telegram polling skipped: missing bot token")
        return
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        while True:
            ...
            if not payload.get("ok"):
                description = str(payload.get("description", "Telegram polling error"))
                logger.warning("Telegram polling returned error: %s", description)
            ...
            for update in payload.get("result", []):
                ...
                try:
                    ...
                    logger.warning("Failed to process Telegram update %s: %s", update_id, exc)
