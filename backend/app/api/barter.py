...
GEMINI_HTTP_TIMEOUT_SECONDS = 30.0
GEMINI_MAX_ATTEMPTS_PER_MODEL = 3
GEMINI_TOTAL_BUDGET_SECONDS = 60.0
...
async def _gemini(prompt: str) -> str:
    ...
    # Use model from settings (default: gemini-flash-latest) with a reliable fallback
        ...
        for model in models:
            ...
            for attempt in range(GEMINI_MAX_ATTEMPTS_PER_MODEL):
                ...
                try:
                    ...
                    last_error = f"Gemini model={model} network error: {exc}"
                ...
                last_error = f"Gemini model={model} status={resp.status_code}: {resp.text[:300]}"
                logger.warning(last_error)
